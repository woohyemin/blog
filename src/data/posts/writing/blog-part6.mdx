...
그래서 결국 utterances로 적용했던 댓글 기능을 giscus로 교체했다.. 사실 조금 귀찮아서 하지 말까 고민도 했었는데,, **utterances은 댓글을 Issues에 저장**하고, **giscus는 댓글을 Discussions에 저장**하기 때문에 나중에 변경하게 될 때 번거로워질 것 같아서 이번에 변경하기로 결정했다.🫠
